...
function printManga(img, id, title) {
  const body = document.querySelector('.manga-container');
  const container = document.createElement('div');
  const pic = document.createElement('img');
  const text = document.createElement('p');
  const aElement = document.createElement('a');
...
function animesAiring() {
  fetch(`https://api.jikan.moe/v3/search/anime?q=&status=airing&order_by=members&sort=desc&limit=20&page=${page.airing}`)
    .then((response) => response.json())
    .then((data) => {
      const { results } = data;
      results.forEach((anime) => {
...
function animesPopular() {
  fetch(`https://api.jikan.moe/v3/search/anime?q=&status=completed&order_by=members&sort=desc&limit=20&page=${page.popular}`)
    .then((response) => response.json())
    .then((data) => {
      const { results } = data;
      results.forEach((anime) => {
...
function topManga() {
  fetch(`https://api.jikan.moe/v3/search/manga?q=&order_by=members&sort=desc&limit=20&page=${page.manga}`)
    .then((response) => response.json())
    .then((data) => {
      const { results } = data;
      results.forEach((anime) => {
        const { image_url, mal_id, title } = anime;
        printManga(image_url, mal_id, title);
      });
    });
}
