...
function moreLikeThis(genres) {
  ...
  fetch(`https://api.jikan.moe/v3/search/anime?q=&genre=${genre}&order_by=members&sort=desc&limit=15&page=${randomizer(1, 5)}`)
    .then((response) => response.json())
    .then((data) => {
      const { results } = data;
      console.log(results);
...
function animeDetail(url) {
  fetch(url)
    ...
    .then((anime) => {
      const {
        status, episodes, score, synopsis, title, title_japanese, trailer_url, rating, genres,
      } = anime;
      console.log(anime);
      console.log(status, episodes, score, synopsis, title, title_japanese, trailer_url, rating, genres);
